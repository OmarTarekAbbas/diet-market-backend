...
class NutritionSpecialistReview extends JsonResourceManager
{
    /**
     ...
     * @const array
     */
    const DATA = ['id', 'review', 'customer', 'rate', 'nutritionSpecialistId', 'orderId', 'published','nutritionSpecialistsReview'];
    ...
    protected function extend($request)
    {
        if (user() && user()->accountType() === 'user') {
            $nutritionSpecialists = repo('nutritionSpecialists')->sharedInfo($this->nutritionSpecialistId);

            $this->set('nutritionSpecialists', $nutritionSpecialists);
        }
    }
}

...
namespace App\Modules\Services\Payments\Methods;
...
use App\Modules\Services\Responses\Payments\HyperPayResponse;
use App\Modules\Services\Exceptions\InvalidPaymentMethodException;

class HyperPay implements PaymentMethodInterface
{
    /**
     * Payment Settings
     *
     ...
     * @throws InvalidPaymentMethodException
     */
    public function saveCreditCard($amount, $paymentMethod, $userInfo = null)
    {
        $user = $userInfo ?? user(); // Current User
     ...
     * @throws InvalidPaymentMethodException
     */
    public function confirmSaveCreditCard(string $hyperPayId, string $paymentMethod): PaymentGatewayResponse
    {
        $options = [
            "query" => [
                "entityId" => $this->getEntityIdOf($paymentMethod),
    ...
    private function getEntityIdOf(string $paymentMethod)
    {
        ...
        if (!in_array($paymentMethod, ['VISA', 'MADA', 'MASTER', 'APPLEPAY'])) {
     ...
     * @throws InvalidPaymentMethodException
     */
    public function confirm(int $orderId, string $hyperPayId, string $paymentMethod): PaymentGatewayResponse
    {
        $paymentMethod = strtoupper($paymentMethod);
        ...
        if ($response->isCompleted() && ($registrationId = $response->get('registrationId')) && !CardRegistration::where('registrationId', $registrationId)->exists()) {
            ...
            if (!$user) {
                ...
                $user = repo('clients')->sharedinfo((int) $order['client']['id']);
            }
